...
import cv2
from filters import gray_filter, threshold_filter, sepia, sobel_filter, cartoon, pencil_scatch
...
def filter_user_image(input_image_path):
    ...
    edited_photo = sepia(input_image_path)
    return edited_photo


def processing_user_photo(update, context):
    # greet_keyboard = ReplyKeyboardMarkup(keyboard)
    # update.message.reply_text('Прислать котика', reply_markup=greet_keyboard)
