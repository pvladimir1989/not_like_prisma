...
class ColorPalette:
    ...
    def from_image(img, n, max_img_size=200, n_init=10):
        img = not_like_prisma.utils.limit_size(img,
                                               max_img_size)  # scale down the image to speedup kmeans
    ...
    def extend(self, extensions):
        extension = [
            not_like_prisma.utils.regulate(
                self.colors.reshape((1, len(self.colors), 3)).astype(np.uint8), *x).reshape(
                (-1, 3)) for x
            in
            extensions]
